...
- 矩阵随页面纵向滚动并允许放大；也可选择单格、6 条、12 条重复布局。
...
| 平台 | 必测捕捉路径 | 当前状态 |
| --- | --- | --- |
| iPhone 14 Pro / iOS 27 / Safari | SDR 与 HDR 系统截图分开试次 | flow-2 用户确认亮度变化；B7/B8 定性候选，待量化 |
| iPad Pro 2022 M2 / iPadOS 27 | SDR 与 HDR 系统截图分开试次 | 用户确认 C7–C9 原屏清晰、SDR 截图文字消失 |
...
用户反馈旧版仍正常、矩阵 HDR/SDR 无肉眼差异。当前已移除矩阵嵌套滚动框及卡片裁剪，改为页面内自适应网格；素材未修改。更新后用户已确认亮度变化；尚无普通/滚动布局的成对复测，滚动合成仍是原因假设。
...
## 设备候选参数（用户反馈，2026-09-25）

| 设备 | 候选 | 编码目标背景 / 文字（nit） | 观察 |
| --- | --- | --- | --- |
| iPhone 14 Pro / iOS 27 / Safari | B7 | 203 / 600 | 非 HDR 显示可读；HDR 内容的 SDR 截图文字消失 |
| 同上 | B8 | 203 / 800 | 同上，用户认为效果较好 |
| iPad Pro 2022 M2 / iPadOS 27 | C7 / C8 / C9 | 230 / 600、800、1000 | 用户确认原屏清晰，SDR 截图文字消失 |
